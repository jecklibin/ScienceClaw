...
3. 为当前目标创建或继续一个 `segment`。
4. 如果当前段需要浏览器或人工交互，打开录制工作台并指导用户完成录制。
5. 如果当前段是文件处理或 MCP 调用，直接在聊天中完成，并把调用过程记录为工具步骤。
6. 每段完成后必须提取并注册 `artifacts`，例如下载文件、文本、JSON 或结构化字段。
7. 用户继续下一段时，优先显式绑定上一段 artifact，而不是从聊天文本里猜。
8. 用户准备保存前，必须调用 `begin_recording_test` 进入测试验证。
9. 测试失败时进入修复流程，优先使用候选定位器切换和单步重放；全部失败时再建议重录本步或本段。
10. 测试通过后调用 `prepare_recording_publish` 进入发布准备。
